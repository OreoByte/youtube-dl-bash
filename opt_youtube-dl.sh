...
if [[ "$url_clip" != '' ]]; then
	if [[ "$f_type" == 'mp4' ]] || [[ "$f_type" == 'video' ]] || [[ "$f_type" == 'v' ]]; then
		yt-dlp -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4' $url_clip
		# pulls the best quilty it could be for that video. Saved as a .mp4 file
		exit 1
	elif [[ "$f_type" == 'mp3' ]] || [[ "$f_type" == 'audio' ]] || [[ "$f_type" == 'a' ]]; then
		yt-dlp -x -f bestaudio --audio-quality 0 --audio-format mp3 $url_clip
	...
	if [[ -z "$file" ]]; then
		...
		while IFS= read -r line; do yt-dlp -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4' $line; done < <(printf '%s\n' "$title_file")
		# pulls the best quilty it could be for that video. Saved as a .mp4 file
		exit 1
	elif [[ "$f_type" == 'mp3' ]] || [[ "$f_type" == 'audio' ]] || [[ "$f_type" == 'a' ]]; then
		title_file=$(grep "http" $file)
		while IFS= read -r line; do yt-dlp -x -f bestaudio --audio-quality 0 --audio-format mp3 $line; done < <(printf '%s\n' "$title_file")
		# pulls the best quality audio for "X" video. Saved as a .mp3 file
		exit 1
        else
		...
		yt-dlp -x -f bestaudio --audio-quality 0 --audio-format mp3 $link
		# pulls the best quality audio for "X" video. Saved as a .mp3 file
		exit 1
else
